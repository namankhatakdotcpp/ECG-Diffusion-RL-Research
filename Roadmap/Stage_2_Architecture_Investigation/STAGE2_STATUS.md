...
| Item 4 -- Gradient competitiveness | In progress -- GPU run completed once, two data-quality issues found and fixed in the script (stale JSON key, missing device/repro-diff logging, fixed in follow-up commit `36a5f0c`); awaiting a clean rerun before verdict is locked. Preliminary (unverified) percentile rank: ~61%. | `546ec26` | -- (pending clean rerun) | `Reports/Item4_PreRegistration.md` | Pending -- `item4_results.tar.gz` (GPU-side, not yet finalized) |
