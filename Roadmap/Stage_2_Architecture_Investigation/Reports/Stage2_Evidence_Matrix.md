...
| Class-embedding received a non-negligible (above-median), actively-optimized gradient during training -- NOT a claim that it rivaled the strongest pathway | Motivates the hypothesis (independent signal) | -- | -- | -- | **Supports the "actively optimized" claim** (61.05th percentile among 95 other tensors, bootstrap 95% CI [58.95%, 62.11%], i.i.d. draws confirmed); **does NOT support "rivals the strongest pathway"** (adaLN is ~6.7x larger -- explicitly not claimed) | Weakly supports (adaLN receives largest gradient bucket, ~6.7x class_emb's mean -- corroborates the "not dominant" half) | -- | -- | -- | **High** (bootstrap-verified tight CI, real checksum/reproducibility values, cross-validated timestep trend against Item 1; two claims kept separate rather than conflated) |
