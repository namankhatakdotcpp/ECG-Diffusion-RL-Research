...
| S3-005 (hybrid) | Implemented, smoke-tested | Not yet trained -- both smoke tests PASS. **Roadmap/implementation discrepancy found 2026-07-06:** `Stage3_Roadmap.md`'s candidate table describes Hybrid as "combination of the above" (LayerScale + Late-block gain + **Residual scaling**). The actual implementation (`model_variants.build_variant_model`, variant `"hybrid"`) combines only `TransformerBlockLayerScale` (blocks 1-4) and `TransformerBlockLayerScaleBoosted` (blocks 5-6, LayerScale + an extra scalar `boost`) -- confirmed by instantiating the model and inspecting each block's class directly. `TransformerBlockResidualScale` (S3-004's mechanism) is NOT present anywhere in Hybrid. This was not a documented design decision; it appears to be a simplification made at implementation time that was never reconciled with the roadmap text. Not changing the implementation now (would invalidate smoke-test provenance and is a training-affecting change); flagging here so this is a known, stated limitation of S3-005's name rather than a silent gap discovered later when comparing results against roadmap intent. | `85e1b1a` / `ea6ce0d` |
