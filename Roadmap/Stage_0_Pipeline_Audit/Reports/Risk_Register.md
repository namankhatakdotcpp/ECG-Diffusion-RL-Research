...
|---|---|---|---|---|
| 14 | The "~380 curated training sequences" described in every prior investigation report corresponds to NO code path anywhere in this repository; corroborated by an independent, unrelated earlier benchmark session (544 steps/epoch measured = 17,418/32 exactly) | **CRITICAL — RESOLVED** | n/a — that's the finding | Real population confirmed as ~17,418 records (full corpus). Entire prior "Investigation Timeline" reclassified as historical narrative, not evidence. `--sanity-check` now authorized against the real corpus. |
...
**Critical findings: 1, resolved same day.** Finding 14 was discovered
while sanity-checking whether the tie-break fix's before/after diff was
computed on the actual training population or a different one — then
resolved by an independent corroborating measurement already sitting in
this project's own history (an unrelated earlier CPU-benchmark session
that measured 544 steps/epoch, exactly matching 17,418/32, with no
awareness the 380-record claim existed). Two independent methods (code
archaeology + runtime measurement) agree with each other and both
contradict the "380 curated sequences" narrative. Resolution: the real
population is ~17,418 records; the entire prior "Investigation Timeline"
(embedding-scale experiment, AdaLN-Zero, CFG sweep, every
conditioning-collapse percentage, AFIB-attractor findings) is
reclassified as historical narrative, not reproducible evidence, until
independently re-derived from a real run in this repository.
**`--sanity-check` is authorized against the real 17,418-record corpus.**

**High findings: 2** — both fixed and re-verified (commits `a9e6047`,
`f8dba53`). Their fix-verification diffs were computed on the full
17,418-record corpus, which is now confirmed (not just assumed) to be the
real, correct population — so those diffs stand as accurate.
